...
describe('alertDelayMs', () => {
  it('returns 0 for load/scroll triggers without a delay', () => {
    ...
    expect(alertDelayMs(makeAlert({trigger: {type: 'scroll', scrollPercent: 30}}))).toBe(0);
...
describe('cooldownAllows', () => {
  ...
  it('shows always-frequency alerts regardless of prior show', () => {
    expect(cooldownAllows(makeAlert({frequency: {type: 'always'}}), undefined, now)).toBe(true);
    expect(cooldownAllows(makeAlert({frequency: {type: 'always'}}), now - 1000, now)).toBe(true);
  });

  it('never repeats a once-frequency alert once shown', () => {
    expect(cooldownAllows(makeAlert({frequency: {type: 'once'}}), undefined, now)).toBe(true);
    expect(cooldownAllows(makeAlert({frequency: {type: 'once'}}), now, now)).toBe(false);
  });

  it('honors cooldownHours before allowing a repeat', () => {
    const alert = makeAlert({frequency: {type: 'always', cooldownHours: 24}});
